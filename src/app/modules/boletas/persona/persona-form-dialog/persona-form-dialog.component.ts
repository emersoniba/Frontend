...
export class PersonaFormDialogComponent implements OnInit{
	...
	dataDepartamento: Departamento[] = [] as Departamento[];

	constructor(
		private fb: FormBuilder,
		private dialogRef: MatDialogRef<PersonaFormDialogComponent>,
	...
	public getDepartamentos(){
		this.departamentoService.getDepartamentos().subscribe({
			next: (response) => {
				...
				this.dataDepartamento = [] as Departamento[];
			}
		});
	}
	...
	public getFormBuilder(): void {
		this.formPersona = this.fb.group({
			...
			apellido_paterno: ['', [Validators.required, Validators.minLength(5)]],
			apellido_materno: ['', [Validators.required, Validators.minLength(5)]],
			cargo: ['', [Validators.required, Validators.minLength(5)]],
			unidad: ['', [Validators.required, Validators.minLength(5)]],
			telefono: ['', [Validators.required, Validators.maxLength(8), Validators.pattern(/^\d+$/)]],
