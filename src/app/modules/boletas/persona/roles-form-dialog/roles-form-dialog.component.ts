...
export class RolesFormDialogComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { ci: number },
  ...
  ngOnInit(): void {
    ...
    this.personaService.getRoles().subscribe((rolesDisponibles) => {
      ...
      this.personaService.getRolesPorPersona(this.data.ci).subscribe((rolesAsignados) => {
        const idsAsignados = (rolesAsignados ?? []).map((r: Rol) => r.id);
